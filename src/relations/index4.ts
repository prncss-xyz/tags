...
export function bindRelations({
	...
	put: <TKey, TValue>(target: IFamily<TKey, TValue>) => (targetKey: TKey, value: TValue) => void
	remove: <TKey, TValue>(target: IFamily<TKey, TValue>) => (targetKey: TKey) => void
	update: <TKey, TValue>(
		target: IFamily<TKey, TValue>,
	) => (targetKey: TKey, modify: (t: TValue) => TValue) => void
}) {
